...
import {createComment, deleteArticle, deleteComment, favoriteArticle, followUser, getArticle, getArticleComments, unfavoriteArticle, unfollowUser,} from '../../../services/conduit';
...
import {CommentSectionState, endSubmittingFavorite, initializeArticlePage, loadArticle, loadComments, MetaSectionState, startDeletingArticle, startSubmittingComment, startSubmittingFavorite, startSubmittingFollow, updateAuthor, updateCommentBody,} from './ArticlePage.slice';
import localizedStrings, {getLocalDate} from "../../../services/localization";
...
export function ArticlePage() {
    ...
    return article.match({
        none: () => <div>{localizedStrings.article.load}</div>,
        some: (article) => (
    ...
    return (
        <Fragment>
            ...
            >
                ...
                &nbsp; {`${following ? localizedStrings.userInfo.unfollow : localizedStrings.userInfo.follow} ${username}`}
            </button>
            &nbsp;
            <button
                className={classObjectToClassName({
            ...
            >
                ...
                &nbsp; {(favorited ? localizedStrings.article.unfavorite : localizedStrings.article.favorite)}
...
function OwnerArticleMetaActions({
    ...
    return (
        <Fragment>
            ...
            >
                <i className='ion-heart'></i>
                &nbsp; {localizedStrings.article.delete}
            </button>
        </Fragment>
    );
}
    ...
    return (
        <div className='row'>
            <div className='col-xs-12 col-md-8 offset-md-2'>
                {user.match({
                    none: () => (
                        <p style={{display: 'inherit'}}>
                            {localizedStrings.formatString(localizedStrings.comment.info,
                                (<Link to='/login'>{localizedStrings.comment.login}</Link>),
                                (<Link to='/register'>{localizedStrings.comment.register}</Link>))}
                        </p>
                    ),
                    some: (user) => (
                        <CommentForm
                ...
                {comments.match({
                    none: () => <div>{localizedStrings.comment.loading}</div>,
                    some: (comments) => (
                        <Fragment>
...
function CommentForm({
    ...
    return (
        ...
        <textarea
            ...
            placeholder={localizedStrings.comment.write}
            rows={3}
        ...
        ></textarea>
            ...
            <div className='card-footer'>
                <img src={image || undefined} className='comment-author-img' alt='#'/>
                <button className='btn btn-sm btn-primary' disabled={submittingComment}>
                    {localizedStrings.comment.post}
                </button>
            </div>
        </form>
    );
    ...
    return (
        <div className='card'>
            ...
            <i className='ion-trash-a'
               aria-label={(localizedStrings.formatString(localizedStrings.comment.delete, (index + 1)).toString())}
