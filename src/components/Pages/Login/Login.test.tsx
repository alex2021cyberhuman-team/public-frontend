...
it('Should show errors if login fails and stop disabling the fields', async () => {
    mockedLogin.mockResolvedValueOnce(Err(new Map<string, string[]>([['email', ['is invalid']], ['password', ['is empty']]])));
    render(<Login/>);
...
it('Should redirect to home if login is successful and setup auth', async () => {
    ...
    expect(axios.defaults.headers.common.Authorization).toMatch('Token jwt.token.here');
